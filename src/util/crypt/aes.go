...
// AesEncrypt AES加密CBC模式
func AesEncrypt(rawData string, key string, iv string) (raw string, err error) {
	defer func() {
...
// AesDecrypt AES解密CBC模式
func AesDecrypt(rawData string, key string, iv string) (raw string, err error) {
	defer func() {
		if errStr := recover(); errStr != nil {
			err = errors.New("请求异常")
		}
