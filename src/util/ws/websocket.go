...
func (manager *Manager) WsClient(ctx *gin.Context) {
	...
	// 测试单个 client 发送数据
	manager.Send(client.Id, client.Group, []byte("Send message --111--"+time.Now().Format("2006-01-02 15:04:05")))
...
func TestSendGroup() {
	for {
		time.Sleep(time.Second * 1)
		WebsocketManager.SendGroup("leffss", []byte("SendGroup message --111--"+time.Now().Format("2006-01-02 15:04:05")))
	}
}
...
func TestSendAll() {
	for {
		...
		WebsocketManager.SendAll([]byte("SendAll message --111--" + time.Now().Format("2006-01-02 15:04:05")))
